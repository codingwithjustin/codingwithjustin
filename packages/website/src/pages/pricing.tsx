...
const PricingMonthly: React.FC = () => {
  const { monthly } = useCheckoutMembership('usd')
  const amount = monthly?.unit_amount
  return (
    <PricingCard>
      <PricingHeading>Monthly Membership</PricingHeading>
      <PricingPrice price={amount ? amount / 100 : 'N / A'} term="month" />
      <PricingFeatures
        features={[
          'Premium videos and courses',
          'Private Discord channel',
          'Removes ads from website'
        ]}
      />
...
const PricingAnnually: React.FC = () => {
  const { yearly, monthly } = useCheckoutMembership('usd')
  const amount = yearly?.unit_amount
  const monthlyAmount = (monthly?.unit_amount ?? 1) * 12
  const discount = amount ? (1 - amount / monthlyAmount) * 100 : 0
  return (
    <PricingCard isPopular colorScheme="green">
      <PricingHeading>Annual Membership</PricingHeading>
      <PricingPrice
        price={amount ? amount / 100 : 'N / A'}
        term="year"
        subtitle={amount ? `$${amount / 100 / 12} / month` : undefined}
      />
      <PricingFeatures
        features={[
...
const PricingLifetime: React.FC = () => {
  const { lifetime } = useCheckoutMembership('usd')
  const amount = lifetime?.unit_amount
  return (
    <PricingCard colorScheme="purple">
      <PricingHeading>Lifetime Membership</PricingHeading>
      <PricingPrice price={amount ? amount / 100 : 'N / A'} />
      <PricingFeatures
        features={[
